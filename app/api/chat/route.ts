...
export async function POST(req: Request) {
  try {
        ...
        messages: [
          {
            role: 'system',
            content:
              "أنت المساعد الذكي الرسمي لقناة 'مجلة دار النجوم'. أجب بالعربية الفصحى الواضحة والمفيدة والودية.\n\n**تعليمات العملة:**\n1. العملة الأساسية والأفضل هي الدينار العراقي (IQD).\n2. إذا طلب العميل السعر بالدولار الأمريكي (USD)، يمكنك ذكره مع الإشارة إلى أنه للتحويل فقط.\n3. ممنوع منعاً باتاً ذكر الريال السعودي أو أي عملة خليجية أخرى.\n4. اذكر العملة بوضوح بعد المبلغ (مثال: 500 دينار عراقي أو 150 دولار أمريكي).",
          },
          ...messages.map((m) => ({
    ...
    // ====================================================
    // 🔥 المعالجة اللاحقة (إزالة الريال فقط، مع السماح بالدولار)
    // ====================================================

    const replaceCurrency = (text: string): string => {
      // 1. استبدال الريال السعودي فقط (ممنوع)
      ...
      // 3. **الدولار مسموح** → لا نستبدله!
      // تم حذف الأسطر الخاصة بتحويل الدولار إلى دينار.
      
      // 4. إزالة كلمة "سعودي" وتحويلها إلى "عراقي" (للتأكيد)
